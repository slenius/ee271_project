...
void write_ppm_file(
  ...
  fprintf(stream, "P6\n%d %d\n255\n", w,
          h);  // Write the file header information

  for (int iy = 0; iy < h;
      iy++) {            // Write the contents of the buffer to file
    for (int ix = 0; ix < w; ix++) {
      // Access pixel (ix, iy) by indexing the appropriate row and then column
      // in the image buffer and taking into account that each pixel has
      // three unsigned char values. This command will write a single pixel
      // value (rgb) to the file in binary format.
